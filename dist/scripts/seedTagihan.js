...
async function main() {
    ...
    try {
        ...
        if (!meter) {
            throw new Error(`Meter tidak ditemukan untuk KoneksiData ${TARGET_KONEKSI_DATA_ID}. ` +
                `Jalankan seed:mongo terlebih dahulu!`);
        }
        console.log(`   ✓ Meter ditemukan: ${meter.NomorMeteran} (ID: ${meter._id})`);
        ...
        if (!kelompok) {
            throw new Error(`KelompokPelanggan tidak ditemukan: ${meter.IdKelompokPelanggan}`);
        }
        console.log(`   ✓ Kelompok: ${kelompok.KodeKelompok} - ${kelompok.NamaKelompok}`);
        console.log(`     Tarif rendah : Rp${kelompok.TarifRendah.toLocaleString()}/m³ (0-${kelompok.BatasRendah} m³)`);
        ...
        console.log(`   ✓ Jan 2026: ${penggunaanJanLiter} liter = ${penggunaanJanM3} m³`);
        console.log(`   ✓ Feb 2026: ${penggunaanFebLiter} liter = ${penggunaanFebM3} m³`);
        console.log(`   ✓ Mar 2026: ${penggunaanMarLiter} liter = ${penggunaanMarM3} m³ (simulasi)`);
        ...
        console.log(`   ✓ Jan: ${penggunaanJanM3} m³ → Biaya Rp${biayaJan.biaya.toLocaleString()} + Beban Rp${kelompok.BiayaBeban.toLocaleString()} = Total Rp${biayaJan.totalBiaya.toLocaleString()}`);
        console.log(`   ✓ Feb: ${penggunaanFebM3} m³ → Biaya Rp${biayaFeb.biaya.toLocaleString()} + Beban Rp${kelompok.BiayaBeban.toLocaleString()} = Total Rp${biayaFeb.totalBiaya.toLocaleString()}`);
        console.log(`   ✓ Mar: ${penggunaanMarM3} m³ → Biaya Rp${biayaMar.biaya.toLocaleString()} + Beban Rp${kelompok.BiayaBeban.toLocaleString()} = Total Rp${biayaMar.totalBiaya.toLocaleString()}`);
        console.log("\n📍 Step 5: Buat/Update Tagihan...");
        const tagihanJan = await Tagihan_1.Tagihan.findOneAndUpdate({ IdMeteran: meter._id, Periode: "2026-01" }, {
            IdMeteran: meter._id,
            Periode: "2026-01",
            PenggunaanSebelum: 0,
        ...
        console.log(`   ✓ Tagihan Jan 2026: LUNAS - Rp${biayaJan.totalBiaya.toLocaleString()} (ID: ${tagihanJan._id})`);
        const tagihanFeb = await Tagihan_1.Tagihan.findOneAndUpdate({ IdMeteran: meter._id, Periode: "2026-02" }, {
            IdMeteran: meter._id,
            Periode: "2026-02",
        ...
        console.log(`   ✓ Tagihan Feb 2026: LUNAS - Rp${biayaFeb.totalBiaya.toLocaleString()} (ID: ${tagihanFeb._id})`);
        const tagihanMar = await Tagihan_1.Tagihan.findOneAndUpdate({ IdMeteran: meter._id, Periode: "2026-03" }, {
            IdMeteran: meter._id,
            Periode: "2026-03",
        ...
        console.log(`   ├──────────────┼────────────┼──────────────────┼────────────┤`);
        console.log(`   │ Jan 2026     │ ${penggunaanJanM3.toString().padEnd(8)} m³│ Rp${biayaJan.totalBiaya.toLocaleString().padEnd(14)}│ LUNAS      │`);
        console.log(`   │ Feb 2026     │ ${penggunaanFebM3.toString().padEnd(8)} m³│ Rp${biayaFeb.totalBiaya.toLocaleString().padEnd(14)}│ LUNAS      │`);
        console.log(`   │ Mar 2026     │ ${penggunaanMarM3.toString().padEnd(8)} m³│ Rp${biayaMar.totalBiaya.toLocaleString().padEnd(14)}│ PENDING    │`);
        console.log(`   └──────────────┴────────────┴──────────────────┴────────────┘`);
        console.log(`\n🔑 Meter ID      : ${meter._id}`);
...
main();
//# sourceMappingURL=seedTagihan.js.map